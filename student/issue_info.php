...
<h1>Information Of Borrowed Books</h1><br><br><br>
</div>
<?php
  if(isset($_SESSION['login_user'])){
...
if(isset($_SESSION['login_user'])){
    ...
    <?php
    $c=0;
    $sql="SELECT student.roll,books.bid,name,authors,edition,issue_book.status,issue,issue_book.return_date FROM
    issue_book join books USING (bid) join student using(roll)
    WHERE issue_book.status in(' ','yes','expired')
